...
void TestOutputStages(int rows, int depth, int cols, int result_offset,
  ...
  OutputStageQuantizeDownInt32ToUint8ScaleByFixedPoint quantize_down_by_fixedpoint_stage;
  quantize_down_by_fixedpoint_stage.result_offset = result_offset;
  quantize_down_by_fixedpoint_stage.result_fixedpoint_multiplier = result_fixedpoint_multiplier;
  quantize_down_by_fixedpoint_stage.result_shift = result_fixedpoint_shift;
  auto quantize_down_by_fixedpoint_pipeline = std::make_tuple(quantize_down_by_fixedpoint_stage);
  Matrix<std::int32_t, ResultOrder> result_quantized_down_by_fixedpoint_int32(rows, cols);
  GemmWithOutputPipeline<std::uint8_t, std::int32_t, DefaultL8R8BitDepthParams>(
      &context, lhs.const_map(), rhs.const_map(), &result_quantized_down_by_fixedpoint_int32,
      lhs_offset, rhs_offset, quantize_down_by_fixedpoint_pipeline);

  std::vector<std::int32_t> diffs_caused_by_fixedpoint;
  for (int r = 0; r < rows; r++) {
    for (int c = 0; c < cols; c++) {
      std::int32_t diff = result_quantized_down_int32(r, c) - result_quantized_down_by_fixedpoint_int32(r, c);
  ...
  Check(diffs_caused_by_fixedpoint[diffs_caused_by_fixedpoint.size() * 1 / 100] == 0);
  Check(diffs_caused_by_fixedpoint[diffs_caused_by_fixedpoint.size() * 99 / 100] == 0);
  ...
  Matrix<std::uint8_t, ResultOrder> result_quantized_down_by_fixedpoint_saturated_uint8(rows,
                                                                          cols);
  GemmWithOutputPipeline<std::uint8_t, std::uint8_t, DefaultL8R8BitDepthParams>(
      &context, lhs.const_map(), rhs.const_map(),
      &result_quantized_down_by_fixedpoint_saturated_uint8, lhs_offset, rhs_offset,
  ...
  for (int r = 0; r < rows; r++) {
    for (int c = 0; c < cols; c++) {
      ...
      std::uint8_t expected = std::min(std::max(quantized, 0), 255);
      Check(expected == result_quantized_down_by_fixedpoint_saturated_uint8(r, c));
    }
  }
